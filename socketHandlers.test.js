...
jest.mock('@aws-sdk/client-s3', () => {
    ...
    class S3Client { constructor() { this.send = sendMock; } }
    class PutObjectCommand { constructor(args) { this.type = 'PutObject'; this.args = args; } }
    class GetObjectCommand { constructor(args) { this.type = 'GetObject'; this.args = args; } }
    return {
        S3Client,
        PutObjectCommand,
        GetObjectCommand,
...
const mockS3Send = require('@aws-sdk/client-s3')._sendMock;
...
describe('Socket.IO Handlers (socketHandlers.js)', () => {
    ...
    describe('join handler', () => {
        it('should join the room and emit historical messages with signed URLs', async () => {
            ...
            expect(mockSocket.emit).toHaveBeenCalledWith('message', expect.objectContaining({ content: 'Old message' }));
