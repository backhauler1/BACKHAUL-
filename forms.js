...
function setupSubmitEvidenceForms() {
    // Use event delegation so this works for forms loaded dynamically via AJAX
    document.body.addEventListener('submit', async (e) => {
        const formElement = e.target;
        
        // Identify evidence forms by checking if they contain the specific file input
        if (formElement && formElement.tagName === 'FORM' && formElement.querySelector('input[name="evidence_file"]')) {
            e.preventDefault();
            ...
            // Optional: Validate that the service date is not in the future
            if (serviceDate && new Date(serviceDate) > new Date()) {
                return showNotification('Service date cannot be in the future.', 'warning');
            }
...
function setupRatingForm(formElement) {
    ...
    formElement.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            formElement.innerHTML = '<p style="color: #28a745; font-weight: bold;">Thank you for your feedback!</p>';
        } catch (error) {
...
function setupPostLoadForm(formElement) {
    ...
    const pickupInput = formElement.querySelector('input[name="pickup"]');
    const deliveryInput = formElement.querySelector('input[name="delivery"]');
    const packageSizeInput = formElement.querySelector('select[name="package_size"]');
    const estimateDisplay = document.getElementById('rateEstimateDisplay');

    const fetchEstimate = async () => {
        const pickup = pickupInput?.value;
        const delivery = deliveryInput?.value;
        const package_size = packageSizeInput?.value || 'Medium';

        if (pickup && delivery && estimateDisplay) {
            estimateDisplay.style.display = 'block';
            estimateDisplay.innerHTML = '<em>Calculating estimate...</em>';

            try {
                const url = new URL(window.location.origin + '/api/loads/estimate');
                url.searchParams.set('pickup', pickup);
                url.searchParams.set('delivery', delivery);
                if (package_size) url.searchParams.set('package_size', package_size);

                const response = await apiFetch(url.toString(), { method: 'GET' });
                
                if (response && response.suggested_rate) {
                    const basisLabel = response.based_on_history
                        ? 'Based on market history'
                        : 'Estimated distance';
                    const basisColor = response.based_on_history
                        ? '#28a745'
                        : '#007bff';
                    const tooltipText = response.based_on_history
                        ? 'This estimate is calculated using the average accepted bids for similar loads in this region.'
                        : 'This estimate is calculated using a standard per-mile rate based on the driving distance and package size.';
                    estimateDisplay.innerHTML = `
                        <span style="color: #28a745; font-weight: 500;">Suggested Rate: $${response.estimated_rate_min} - $${response.estimated_rate_max}</span> 
                        <span style="font-size: 0.85em;">(${response.distance_miles} miles)</span>
                        <br>
                        <em title="${tooltipText}" style="font-size: 0.8em; color: ${basisColor}; cursor: help; border-bottom: 1px dotted ${basisColor};">${basisLabel}</em>
                    `;
                }
            } catch (error) {
                estimateDisplay.innerHTML = `<span style="color: #dc3545; font-size: 0.85em;">Could not calculate estimate: ${error.message}</span>`;
            }
        }
    };

    if (pickupInput) pickupInput.addEventListener('blur', fetchEstimate);
    if (deliveryInput) deliveryInput.addEventListener('blur', fetchEstimate);
    if (packageSizeInput) packageSizeInput.addEventListener('change', fetchEstimate);
    // ----------------------------
...
function setupCheckoutForm(formElement) {
    ...
    // --- TEMPORARY: Disable Promo Code Functionality ---
    // This will hide the promo code input field from the user interface.
    ...
    formElement.addEventListener('submit', async (e) => {
        ...
        // Abort early if the button is visually disabled (e.g., user is unverified)
