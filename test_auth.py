...
def test_logout_from_all_devices(app, add_user):
    """
    GIVEN a user is logged in on two separate "devices" (clients)
    WHEN they "logout from all devices" on one client
    THEN both clients should be logged out.
    """
    # 1. Create a user
    user = add_user(username="multi_device_user", email="multi@example.com", password="pw")

    # 2. Create two separate test clients from the app fixture
    client_a = app.test_client()
    client_b = app.test_client()

    # 3. Log in on both "devices" and verify they can access a protected route
    with client_a:
        client_a.post('/login', data={'username': user['username'], 'password': user['password']})
        dashboard_a_before = client_a.get('/dashboard')
        assert dashboard_a_before.status_code == 200

    with client_b:
        client_b.post('/login', data={'username': user['username'], 'password': user['password']})
        dashboard_b_before = client_b.get('/dashboard')
        assert dashboard_b_before.status_code == 200

    # 4. On Device A, trigger "logout from all devices"
    with client_a:
        logout_all_response = client_a.post('/logout-all', follow_redirects=True)
        assert logout_all_response.status_code == 200
        assert b"Login" in logout_all_response.data

    # 5. On Device B, attempt to access the dashboard again
    with client_b:
        dashboard_b_after = client_b.get('/dashboard', follow_redirects=False)
        assert dashboard_b_after.status_code == 302
        assert '/login' in dashboard_b_after.location

def test_dashboard_role_content(client, add_user, app):
    """
    GIVEN a shipper and a transporter with specific database entries
    WHEN they access the dashboard
    THEN they see content specific to their roles and not the other's content.
    """
    # 1. Create the users using the factory, specifying their roles
    shipper = add_user(username="shipper1", email="shipper1@example.com", password="pw", role="shipper")
    transporter = add_user(username="transporter1", email="transporter1@example.com", password="pw", role="transporter")

    # 2. Add specific data for each to the database
    with app.app_context():
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
        # Add a mock load for the shipper
        cursor.execute(
            "INSERT INTO loads (load_id, user_id, description, timestamp, payment_status, is_flagged) VALUES (?, ?, ?, ?, ?, ?)",
            ("test-load-id", shipper['user_id'], "Super Secret Shipper Cargo", 1234567890.0, "unpaid", 0)
        )
        # Add a mock vehicle for the transporter
        cursor.execute(
            "INSERT INTO vehicles (vehicle_id, user_id, vehicle_type, timestamp) VALUES (?, ?, ?, ?)",
            ("test-veh-id", transporter['user_id'], "Mega Transporter Truck", 1234567890.0)
        )
        conn.commit()
        conn.close()

    # 3. Test Shipper Dashboard
    with client:
        client.post('/login', data={'username': shipper['username'], 'password': shipper['password']})
        response_shipper = client.get('/dashboard')
        
        assert response_shipper.status_code == 200
        assert b"Super Secret Shipper Cargo" in response_shipper.data
        assert b"Mega Transporter Truck" not in response_shipper.data
        
        client.get('/logout') # Log out to clear the session for the next test

    # 4. Test Transporter Dashboard
    with client:
        client.post('/login', data={'username': transporter['username'], 'password': transporter['password']})
        response_transporter = client.get('/dashboard')
        
        assert response_transporter.status_code == 200
        assert b"Mega Transporter Truck" in response_transporter.data
        assert b"Super Secret Shipper Cargo" not in response_transporter.data