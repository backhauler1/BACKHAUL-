...
describe('Cron Jobs', () => {
    ...
    describe('notifyIncompleteProfiles (0 10 * * *)', () => {
        ...
        it('should send reminder emails to drivers with incomplete profiles', async () => {
            ...
            expect(pool.query).toHaveBeenNthCalledWith(1, expect.stringContaining("LEFT JOIN trucks"));
            expect(pool.query).toHaveBeenNthCalledWith(2, expect.stringContaining("LEFT JOIN companies"));
    ...
    describe('cleanupUnverifiedUsers (0 4 * * *)', () => {
        let cleanupJob;
        beforeEach(() => {
            cleanupJob = scheduledJobs['0 4 * * *'];
        });

        it('should delete old, unverified users and log the count', async () => {
            ...
            expect(logger.info).toHaveBeenCalledWith('[Cron] cleanupUnverifiedUsers: Cleaned up 3 old, unverified user accounts.');
        });

        it('should log that no users were cleaned up if rowCount is 0', async () => {
            pool.query.mockResolvedValue({ rowCount: 0 });
            await cleanupJob();
            expect(logger.info).toHaveBeenCalledWith('[Cron] cleanupUnverifiedUsers: No unverified user accounts to clean up.');
        });
        ...
        it('should log an error if the database query fails', async () => {
            ...
            await cleanupJob();
            expect(logger.error).toHaveBeenCalledWith(
                expect.stringContaining('cleanupUnverifiedUsers failed'),
