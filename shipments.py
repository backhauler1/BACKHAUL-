...
def post_load():
    ...
    signature_required = 1 if request.form.get('signature_required') else 0
    ...
    cursor.execute(
        "INSERT INTO loads (load_id, user_id, description, weight, dimensions, pickup, delivery, offer, shipping_date, timestamp, ip_address, is_flagged, signature_required) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (load_id, user_id, description, weight, dimensions, pickup, delivery, offer, shipping_date, time.time(), ip_address, is_flagged, signature_required)
    )
    conn.commit()
    conn.close()
...
def match_load():
    ...
    cursor.execute("SELECT l.load_id, l.description, u.email, u.username FROM loads l JOIN users u ON l.user_id = u.user_id WHERE l.load_id = ?", (load_id,))
    load_data = cursor.fetchone()
    if not load_data:
        conn.close()
        return jsonify({"error": f"Load ID '{load_id}' not found."}), 404
        
    cursor.execute("SELECT v.vehicle_id, v.vehicle_type, u.email, u.username FROM vehicles v JOIN users u ON v.user_id = u.user_id WHERE v.vehicle_id = ?", (vehicle_id,))
    vehicle_data = cursor.fetchone()
    if not vehicle_data:
        conn.close()
        return jsonify({"error": f"Vehicle ID '{vehicle_id}' not found."}), 404

    # Prevent double-matching! Check if load or vehicle is already in an active match
    cursor.execute("SELECT match_id FROM matches WHERE load_id = ? OR vehicle_id = ?", (load_id, vehicle_id))
    if cursor.fetchone():
        conn.close()
        return jsonify({"error": "This load or vehicle has already been matched."}), 400
    ...
    shipper_email, shipper_username = load_data[2], load_data[3]
    driver_email, driver_username = vehicle_data[2], vehicle_data[3]
    load_description = load_data[1]

    if shipper_email:
        send_email(shipper_email, "Load Matched!", f"Great news! Your load '{load_description}' has been successfully matched with transporter '{driver_username}'. Please check your dashboard for details.")
    if driver_email:
        send_email(driver_email, "Vehicle Matched!", f"Great news! Your vehicle has been successfully matched to transport '{load_description}' for shipper '{shipper_username}'. Please check your dashboard for details.")
