...
def find_matches():
    ...
    for vehicle_id, truck in trucks_db.items():
        ...
        load_delivery = load.get('delivery')
        truck_start = truck.get('departure_city')
        truck_end = truck.get('destination_city')
        
        if load_pickup and load_delivery and truck_start and truck_end:
            if GEOPY_AVAILABLE:
                pickup_coords = get_coordinates(load_pickup)
                delivery_coords = get_coordinates(load_delivery)
                start_coords = get_coordinates(truck_start)
                end_coords = get_coordinates(truck_end)

                if all([pickup_coords, delivery_coords, start_coords, end_coords]):
                    # Calculate the direct route distance for the truck
                    truck_route_dist = geodesic(start_coords, end_coords).miles
                    
                    # Calculate distance if truck detours: Start -> Pickup -> Delivery -> End
                    detour_dist = (geodesic(start_coords, pickup_coords).miles + 
                                   geodesic(pickup_coords, delivery_coords).miles + 
                                   geodesic(delivery_coords, end_coords).miles)
                                   
                    # Consider it a match if the detour isn't drastically longer than the direct route
                    # e.g., Detour is within 20% of original distance + 50 miles buffer
                    if detour_dist <= (truck_route_dist * 1.20) + 50:
                        route_match = True
            
            # Fallback to exact city match if geocoding fails or geopy isn't installed
            if not route_match:
                if load_pickup.strip().lower() == truck_start.strip().lower() and \
                   load_delivery.strip().lower() == truck_end.strip().lower():
                    route_match = True
...
def match_load():
    ...
    # Save the match permanently in the SQLite database
