...
def test_shipper_acknowledge_delivery_success(client, app, add_user, create_load, monkeypatch):
    ...
    # 2. Simulate transporter submitting a POD (e.g., a signature)
    with client:
        with client.session_transaction() as s:
            s['user_id'] = transporter['user_id']
            s['username'] = transporter['username']
        ...
        pod_data = { 'load_id': load['load_id'], 'signature': 'Transporter Signature' }
        client.post('/delivery-confirmation', data=pod_data, content_type='multipart/form-data')

    # 3. Log in as the shipper and acknowledge the delivery
    with client:
        with client.session_transaction() as s:
            s['user_id'] = shipper['user_id']
            s['username'] = shipper['username']
            s['role'] = shipper['role']
        
        response = client.post(f'/shipper-acknowledge-delivery/{load["load_id"]}')

        # 4. Assert the response
        assert response.status_code == 200
        assert b"Delivery successfully acknowledged by shipper." in response.data
        ...
        with app.app_context():
            ...
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute("SELECT shipper_acknowledged, shipper_acknowledgment_timestamp FROM loads WHERE load_id = ?", (load['load_id'],))
            load_entry = cursor.fetchone()
            conn.close()

        assert load_entry is not None
        assert load_entry['shipper_acknowledged'] == 1
        assert load_entry['shipper_acknowledgment_timestamp'] is not None

def test_shipper_acknowledge_delivery_unauthorized(client, app, add_user, create_load):
    """
    GIVEN a load
    WHEN a user who is not the shipper attempts to acknowledge delivery
    THEN the request should be rejected as unauthorized.
    """
    shipper = add_user("shipper_unauth", "shipper_unauth@example.com", "pw", role="shipper")
    other_user = add_user("other_user", "other@example.com", "pw", role="transporter")
    load = create_load(shipper['user_id'])

    with client:
        with client.session_transaction() as s:
            s['user_id'] = other_user['user_id']
            s['username'] = other_user['username']
            s['role'] = other_user['role']
        
        response = client.post(f'/shipper-acknowledge-delivery/{load["load_id"]}')

        assert response.status_code == 403
        assert b"Unauthorized: Only the shipper can acknowledge this delivery." in response.data

def test_shipper_acknowledge_delivery_already_acknowledged(client, app, add_user, create_load):
    """
    GIVEN a load that has already been acknowledged by the shipper
    WHEN the shipper attempts to acknowledge delivery again
    THEN the request should be rejected.
    """
    shipper = add_user("shipper_already_ack", "shipper_already_ack@example.com", "pw", role="shipper")
    load = create_load(shipper['user_id'], shipper_acknowledged=1) # Load is already acknowledged
    ...
    with client:
        ...
        assert response.status_code == 400
        assert b"This delivery has already been acknowledged by the shipper." in response.data

def test_shipper_acknowledge_delivery_load_not_found(client, app, add_user):
    """
    GIVEN a non-existent load ID
    WHEN a shipper attempts to acknowledge delivery
    THEN the request should return a 404.
    """
    shipper = add_user("shipper_no_load", "shipper_no_load@example.com", "pw", role="shipper")
    ...
    with client:
        ...
        response = client.post('/shipper-acknowledge-delivery/non_existent_load_id')
