...
export async function startDriverTracking(loadId, destLat, destLng, truckId) {
    ...
    if (typeof io !== 'undefined' && !socket) { // Initialize only once
        socket = io();
    }
    if (socket) {
        socket.emit('join_tracking_room', { loadId });
    }
    ...
    // Sync location to the backend and broadcast via WebSocket
    if (truckId) {
        let isSyncing = false;
        // Reduced interval from 30s to 5s for smoother live tracking
        locationUpdateInterval = setInterval(async () => {
            if (lastKnownPosition && !isSyncing) {
                isSyncing = true;
                
                // Broadcast real-time location to anyone watching this load
                if (socket && socket.connected) {
                    socket.emit('driver_location_update', { loadId, truckId, ...lastKnownPosition });
                }
        ...
        }, 5000); // Send updates every 5 seconds instead of 30
    }
}
