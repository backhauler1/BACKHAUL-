...
describe('Password Forms Validation (passwordForms.js)', () => {
    beforeEach(() => {
        ...
        // Mock window.location to simulate arriving via an email link with tokens
        Object.defineProperty(window, 'location', {
            writable: true,
            // Use a full URL object to make URLSearchParams work correctly in JSDOM
            value: new URL('http://localhost/?token=mock_token_123&email=test@example.com')
        });
    ...
    describe('Reset Password Form (Initialization)', () => {
        it('should show an error if token or email is missing from URL', () => {
            // Override the beforeEach setup for this specific test
            Object.defineProperty(window, 'location', {
                writable: true,
    ...
    describe('Change Password Form', () => {
        ...
        it('should submit successfully and trigger redirect on success', async () => {
            jest.useFakeTimers();
            const form = document.getElementById('change-password-form');
            apiFetch.mockResolvedValueOnce({ message: 'Password changed.' });
