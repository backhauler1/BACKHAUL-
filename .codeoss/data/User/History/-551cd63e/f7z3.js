...
router.get('/admin/top-referrers', protect, authorize('admin'), validate({ query: paginationQuerySchema }), async (req, res) => {
    ...
    try {
        // Count total number of unique referrers for pagination
        const countQuery = 'SELECT COUNT(DISTINCT referred_by_id) FROM users WHERE referred_by_id IS NOT NULL';
        const { rows: countRows } = await pool.query(countQuery);
        const totalItems = parseInt(countRows[0].count, 10);
        const totalPages = Math.ceil(totalItems / limit) || 1;

        // Fetch the referrers, count their referrals, and sort descending
        const queryStr = `
            SELECT u.id, u.name, u.email, u.referral_code, COUNT(r.id)::int AS total_referrals
            FROM users u
...
router.get('/me/referrals', protect, async (req, res) => {
    ...
    try {
        ...
        // Count how many users have this user's ID as their referred_by_id
        const statsQuery = await pool.query('SELECT COUNT(*) FROM users WHERE referred_by_id = $1', [userId]);
