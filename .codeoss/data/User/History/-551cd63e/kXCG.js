...
const { withRetry } = require('./dbUtils'); // Assuming dbUtils.js is in the same directory
...
router.get('/', protect, authorize('admin'), validate({ query: paginationQuerySchema }), async (req, res) => {
    ...
    try {
        ...
        const { rows: countRows } = await withRetry(
...
router.delete('/me', protect, async (req, res) => {
    ...
    try {
        // 1. Check for active loads (either as the shipper/owner or the driver)
        const activeLoadsQuery = `
            SELECT id FROM loads 
            WHERE (owner_id = $1 OR driver_id = $1) 
              AND (status IS NULL OR status IN ('available', 'assigned', 'en_route', 'arrived', 'loading_completed'))
            LIMIT 1
        `;
        const { rows: activeLoads } = await pool.query(activeLoadsQuery, [userId]);
        if (activeLoads.length > 0) {
            return res.status(400).json({ message: 'Cannot delete account with active loads. Please complete or cancel them first.' });
        }
        ...
        const pendingOrdersQuery = `
            ...
            LIMIT 1
        `;
        const { rows: pendingOrders } = await pool.query(pendingOrdersQuery, [userId]);
        if (pendingOrders.length > 0) {
            return res.status(400).json({ message: 'Cannot delete account with pending payments. Please resolve them first.' });
        }

        // 3. Anonymize user data to preserve referential integrity (e.g., order history) 
 ...
 * Exports all personal data for the current user as a JSON file (GDPR Right to Data Portability).
 */
router.get('/me/export', protect, async (req, res) => {
    ...
    try {
        ...
        const userQuery = await pool.query(
            'SELECT id, name, email, roles, created_at, preferred_locale, referral_code, consent_log, penalty_count FROM users WHERE id = $1',
            [userId]
        );
        ...
        // Run remaining queries concurrently for better performance
        const [ownedLoads, drivenLoads, orders, reviewsGiven, reviewsReceived] = await Promise.all([
            pool.query('SELECT * FROM loads WHERE owner_id = $1', [userId]),
            pool.query('SELECT * FROM loads WHERE driver_id = $1', [userId]),
            pool.query('SELECT * FROM orders WHERE user_id = $1', [userId]),
            pool.query('SELECT * FROM load_ratings WHERE rater_id = $1', [userId]),
            pool.query('SELECT * FROM load_ratings WHERE target_id = $1', [userId])
        ]);
        ...
        const exportData = {
            ...
            profile: userProfile,
            activity: {
                loadsPosted: ownedLoads.rows,
                loadsDriven: drivenLoads.rows,
                financialOrders: orders.rows,
                reviewsGiven: reviewsGiven.rows,
                reviewsReceived: reviewsReceived.rows
            }
        };

        // 1. Gather all S3 file URLs associated with the user
        const fileUrls = new Set();
        
        // Get BOLs from loads
        const loadsQuery = await pool.query('SELECT bol_url, signed_bol_url FROM loads WHERE owner_id = $1 OR driver_id = $1', [userId]);
        loadsQuery.rows.forEach(row => {
            if (row.bol_url) fileUrls.add(row.bol_url);
            if (row.signed_bol_url) fileUrls.add(row.signed_bol_url);
        });

        // Get company thumbnails and compliance documents
        const companiesQuery = await pool.query('SELECT id, thumbnail_url FROM companies WHERE owner_id = $1', [userId]);
        const companyIds = [];
        companiesQuery.rows.forEach(row => {
            companyIds.push(row.id);
            if (row.thumbnail_url) fileUrls.add(row.thumbnail_url);
        });

        if (companyIds.length > 0) {
            const docsQuery = await pool.query('SELECT file_url FROM company_documents WHERE company_id = ANY($1)', [companyIds]);
            docsQuery.rows.forEach(row => {
                if (row.file_url) fileUrls.add(row.file_url);
            });
        }

        // 2. Initialize the ZIP archive and prompt download
        res.setHeader('Content-Disposition', 'attachment; filename="user_data_export.zip"');
        ...
        // 3. Append the structured JSON data
        archive.append(JSON.stringify(exportData, null, 2), { name: 'user_data.json' });

        // 4. Stream all associated files from S3 into the ZIP archive
        for (const url of fileUrls) {
            try {
                const s3Key = decodeURIComponent(new URL(url).pathname.substring(1));
                const filename = s3Key.split('/').pop() || 'unknown_file';
                
                const s3Response = await s3.send(new GetObjectCommand({ Bucket: process.env.AWS_S3_BUCKET_NAME, Key: s3Key }));
                archive.append(s3Response.Body, { name: `files/${filename}` });
            } catch (s3Err) {
                console.error(`Failed to export file ${url}:`, s3Err);
                archive.append(`Error downloading file: ${url}\n${s3Err.message}`, { name: `files/ERROR_log.txt` });
            }
        }

        // 5. Finalize the archive (this completes the response stream)
        await archive.finalize();
    } catch (error) {
        console.error('Data Export Error:', error);
        res.status(500).json({ message: 'Internal server error during data export.' });
    }
});
...
/**
 * GET /api/users/suspension-history
 * Fetches the suspension history for admin review.
 */
router.get('/suspension-history', protect, authorize('admin'), validate({ query: paginationQuerySchema }), async (req, res) => {
    const { page, limit, search } = req.query;
    const offset = (page - 1) * limit;

    try {
        let countQuery = 'SELECT COUNT(*) FROM suspension_logs';
        let queryStr = `
            SELECT sl.created_at, u.name AS target_user_name, sl.target_user_id, a.name AS admin_name, sl.admin_id, sl.action, sl.reason
            FROM suspension_logs sl
            JOIN users u ON sl.target_user_id = u.id
            JOIN users a ON sl.admin_id = a.id
        `;
        ...
        if (search) {
            const whereClause = ` WHERE u.name ILIKE $1`;
        ...
        queryStr += ` ORDER BY sl.created_at DESC LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`;
        queryParams.push(limit, offset);

        const { rows: history } = await pool.query(queryStr, queryParams);
        ...
        console.error('Fetch Suspension History Error:', error);
 ...
 * Exports the suspension history as CSV.
 */
router.post('/suspension-history/export', protect, authorize('admin'), async (req, res) => {
    ...
    try {
        ...
        const queryParams = [];
        if (search) {
            queryStr += ` WHERE u.name ILIKE $1`;
            queryParams.push(`%${search}%`);
        }
        queryStr += ` ORDER BY sl.created_at DESC`;

        const stream = client.query(queryStr, queryParams);

        res.setHeader('Content-Type', 'text/csv');
        res.setHeader('Content-Disposition', 'attachment; filename="suspension-history.csv"');
        res.write('Date,Target User,Admin,Action,Reason\n');

        stream.on('data', (row) => {
            res.write(`"${new Date(row.created_at).toISOString()}","${row.target_user_name} (#${row.target_user_id})","${row.admin_name} (#${row.admin_id})","${row.action}","${row.reason || ''}"\n`);
        });
        ...
        res.status(500).json({ message: 'Internal server error during CSV export.' });
        if (client) {
            client.release();
        }
    }
