...
export function initializeInvoicesPage() {
    ...
    if (!document.getElementById('invoices-search-container')) {
        ...
        searchContainer.innerHTML = `
            <input type="text" id="invoices-search-input" placeholder="Search by Order ID or Status..." style="padding: 8px; border-radius: 4px; border: 1px solid #ccc; font-size: 0.9em; flex: 1; max-width: 300px;">
            <select id="invoices-sort" style="padding: 8px; border-radius: 4px; border: 1px solid #ccc; font-size: 0.9em; cursor: pointer; margin-left: 10px;">
...
async function fetchAndRenderInvoices(page = 1) {
    ...
    try {
        ...
        const { data: orders, pagination } = await apiFetch(`/api/orders/history?page=${page}&limit=10&search=${encodeURIComponent(searchQuery)}&sortBy=${sortBy}&sortOrder=${sortOrder}`);
