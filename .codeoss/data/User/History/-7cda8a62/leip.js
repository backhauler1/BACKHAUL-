    ...
    if (!document.getElementById('invoices-search-container')) {
        ...
        searchContainer.innerHTML = `
            <div style="display: flex; gap: 10px; flex: 1; max-width: 500px;">
                <input type="text" id="invoices-search-input" placeholder="Search by Order ID or Status..." style="padding: 8px; border-radius: 4px; border: 1px solid #ccc; font-size: 0.9em; flex: 1;">
        ...
        document.getElementById('invoices-export-btn').addEventListener('click', async (e) => {
            ...
            try {
                const blob = await apiFetch(`/api/orders/history/export?search=${encodeURIComponent(searchQuery)}&sortBy=${sortBy}&sortOrder=${sortOrder}`, {
                    method: 'GET',
                    responseType: 'blob'
                });
