...
export async function exportSuspensionHistoryCSV() {
    try {
        let url = `/api/users/suspension-history/export`;
        if (currentHistorySearchTerm) {
            url += `?search=${encodeURIComponent(currentHistorySearchTerm)}`;
        }

        // Use a direct anchor link to trigger the download.
        // This allows the browser to stream the file directly to disk, avoiding memory issues.
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', ''); // The backend should set the exact filename via Content-Disposition
