...
const mockSingle = jest.fn().mockImplementation(() => (req, res, next) => {
    if (mockMulterError) return next(mockMulterError);
    if (!req.headers['x-no-file']) {
        req.file = { location: 'https://mock-bucket.s3.amazonaws.com/companies/thumbnails/mock-image.jpg' };
    }
    next();
});
...
jest.mock('./uploads', () => ({
    ...
    uploadDocument: { single: mockSingle }
}), { virtual: true });

multer.mockReturnValue({ single: mockSingle });

// 5. Mock the geocoding service
...
app.use((err, req, res, next) => {
    ...
    res.status(err.status || 400).json({ message: err.message || 'Internal Server Error' });
});
...
describe('Companies API - Upload Routes', () => {
    ...
    it('should successfully register a company and mock the S3 file upload', async () => {
        ...
        const res = await request(app)
            ...
            .send({ companyName: 'Test Company', description: 'A mock company description.', location: 'New York, NY', privacyPolicy: 'on' });
...
describe('Companies API - PUT /:id', () => {
    it('should update a company and delete the old thumbnail when a new one is uploaded', async () => {
        ...
        const res = await request(app)
            ...
            .send({ companyName: 'Updated Company Name', privacyPolicy: 'on' });
    ...
    it('should return 404 if the company does not exist', async () => {
        ...
        const res = await request(app).put('/api/companies/999').send({ companyName: 'Updated Company Name', privacyPolicy: 'on' });
...
describe('Companies API - Compliance Documents', () => {
    ...
    describe('POST /:id/documents', () => {
        ...
        it('should return 400 if no file is uploaded', async () => {
            ...
            expect(res.body.message).toBe('No document uploaded. Please attach a valid file.');
        });
    });
...
describe('Companies API - Multer Error Handling', () => {
    it('should return 400 if file is too large', async () => {
        ...
        const res = await request(app)
            .post('/api/companies/register')
            .send({ companyName: 'Test Company', location: 'NY', privacyPolicy: 'on' });
    ...
    it('should return 400 for an invalid file type', async () => {
        mockMulterError = new Error('Invalid file format. Please upload a JPEG, PNG, GIF, or WebP.');
