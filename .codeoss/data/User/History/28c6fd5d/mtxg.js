...
const directionsService = mbxDirections({ accessToken: process.env.MAPBOX_TOKEN });
...
/**
 * GET /api/loads/estimate
 * Provides an estimated shipping rate based on driving distance and size.
 */
router.get('/estimate', validate({ 
    query: z.object({ 
    ...
    const { pickup, delivery, package_size } = req.query;

    try {
        // 1. Geocode pickup and delivery to get coordinates
        const pickupGeo = await geocodingService.forwardGeocode({ query: pickup, limit: 1 }).send();
        const deliveryGeo = await geocodingService.forwardGeocode({ query: delivery, limit: 1 }).send();

        if (!pickupGeo.body.features.length || !deliveryGeo.body.features.length) {
            return res.status(400).json({ message: 'Could not find coordinates for the provided addresses.' });
        }

        const pickupCoords = pickupGeo.body.features[0].center;
        const deliveryCoords = deliveryGeo.body.features[0].center;

        // Extract the Region (State) to use for matching past history
        const getRegion = (feature) => {
            const context = feature.context || [];
            const regionObj = context.find(c => c.id.startsWith('region'));
            return regionObj ? regionObj.text : feature.text;
        };
        const pickupRegion = getRegion(pickupGeo.body.features[0]);
        const deliveryRegion = getRegion(deliveryGeo.body.features[0]);

        // Query Database for Historical Averages
        // Look for accepted bids on loads matching similar pickup/delivery regions
        const historyQuery = `
            SELECT AVG(b.bid_amount) as avg_rate, COUNT(b.id) as job_count
            FROM bids b
            JOIN loads l ON b.load_id = l.id
            WHERE b.status = 'accepted'
              AND l.pickup_address ILIKE $1
              AND l.delivery_address ILIKE $2
        `;
        const historyResult = await pool.query(historyQuery, [`%${pickupRegion}%`, `%${deliveryRegion}%`]);
        const historicalData = historyResult.rows[0];

        // 2. Get driving distance using Mapbox Directions
        const dirResponse = await directionsService.getDirections({
            profile: 'driving',
            waypoints: [{ coordinates: pickupCoords }, { coordinates: deliveryCoords }]
        }).send();

        if (!dirResponse.body.routes.length) {
            return res.status(400).json({ message: 'Could not calculate a driving route.' });
        }

        const distanceMiles = dirResponse.body.routes[0].distance * 0.000621371; // Convert meters to miles

        // 3. Calculate estimated rate
        let suggestedRate;
        let isHistorical = false;

        // If we have at least 3 historical jobs for this route, use the historical average
        if (historicalData && parseInt(historicalData.job_count, 10) >= 3) {
            suggestedRate = parseFloat(historicalData.avg_rate);
            isHistorical = true;
        } else {
            // Fallback: Calculate based on base per-mile rates
            const ratePerMileMap = { 'Small': 1.10, 'Medium': 1.75, 'Large': 2.50 };
            const ratePerMile = ratePerMileMap[package_size] || 1.75;
            const baseFee = package_size === 'Large' ? 100 : 40; // Base show-up fee
            suggestedRate = (distanceMiles * ratePerMile) + baseFee;
        }

        res.status(200).json({
            distance_miles: Math.round(distanceMiles),
            estimated_rate_min: Math.round(suggestedRate * 0.85), // -15% margin
            estimated_rate_max: Math.round(suggestedRate * 1.15), // +15% margin
            suggested_rate: Math.round(suggestedRate),
            based_on_history: isHistorical
        });
    } catch (error) {
        console.error('Rate Estimation Error:', error);
        res.status(500).json({ message: 'Internal server error while estimating rate.' });
    }
});
...
router.get('/posted', protect, validate({ query: paginationQuerySchema.extend({
    ...
    try {
        let baseWhere = 'WHERE owner_id = $1';
        const queryParams = [ownerId];
        let paramIndex = 2;

        // Apply search filter safely via parameterized query
        if (search) {
            baseWhere += ` AND title ILIKE $${paramIndex}`;
        ...
        const countQuery = `SELECT COUNT(*) FROM loads ${baseWhere}`;
        const { rows: countRows } = await pool.query(countQuery, queryParams);
        const totalItems = parseInt(countRows[0].count, 10);
...
router.patch('/:id/eta', protect, validate({ params: numericParamSchema('id'), body: etaSchema }), async (req, res) => {
    ...
    try {
        ...
        if (shipper) {
            ...
            await sendPushNotification(updatedLoad.owner_id, {
                title: 'Driver ETA Updated',
                body: `The driver for "${updatedLoad.title}" updated their ETA to ${formattedEta}.`,
                url: `/loads/${loadId}/status`
            });
        }
...
router.post('/:id/bids/:bidId/accept', protect, validate({ params: z.object({ id: z.coerce.number().int().positive(), bidId: z.coerce.number().int().positive() }) }), async (req, res) => {
    ...
    try {
        ...
        if (driver) {
             ...
             // Send a push notification to the driver's device natively
             sendPushNotification(bid.driver_id, {
                 title: 'Bid Accepted!',
                 body: `Your bid of $${bid.bid_amount} for "${load.title}" has been accepted.`,
                 url: `/loads/${loadId}/status`
             });
        }
