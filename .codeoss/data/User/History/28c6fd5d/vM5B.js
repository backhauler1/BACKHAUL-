...
router.post('/:id/rate', protect, validate({ params: numericParamSchema('id'), body: rateSchema }), async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            await client.query(
                'INSERT INTO load_ratings (load_id, rater_id, target_id, rating) VALUES ($1, $2, $3, $4)',
                [loadId, currentUserId, targetUserId, rating]
            );
            ...
            const updateQuery = `
                ...
                WHERE id = $2
            `;
            await client.query(updateQuery, [rating, targetUserId]);
