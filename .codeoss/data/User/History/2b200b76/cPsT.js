...
/**
 * Deletes old, unverified user accounts.
 * An account is considered unverified if it was created more than 7 days ago
 * and the user has never logged in. It also checks that they have no associated
 * companies or loads to prevent deleting potentially important data.
 */
async function cleanupUnverifiedUsers() {
    const jobName = 'cleanupUnverifiedUsers';
    logger.info(`[Cron] Starting job: ${jobName}`);
        ...
        const result = await pool.query(
            `DELETE FROM users
             WHERE last_login_at IS NULL
               AND created_at < $1
               AND id NOT IN (SELECT DISTINCT owner_id FROM companies WHERE owner_id IS NOT NULL)
               AND id NOT IN (SELECT DISTINCT owner_id FROM loads WHERE owner_id IS NOT NULL)`,
            [sevenDaysAgo.toISOString()]
        );

        if (result.rowCount > 0) {
            logger.info(`[Cron] ${jobName}: Cleaned up ${result.rowCount} old, unverified user accounts.`);
        } else {
            logger.info(`[Cron] ${jobName}: No unverified user accounts to clean up.`);
        }
    } catch (error) {
        logger.error(`[Cron] Job ${jobName} failed:`, { error: error.message, stack: error.stack });
    }
...
function startCronJobs() {
    ...
    // Schedule to run once every day at 4:00 AM server time.
    cron.schedule('0 4 * * *', cleanupUnverifiedUsers);
