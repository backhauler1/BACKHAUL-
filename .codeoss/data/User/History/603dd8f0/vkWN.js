...
describe('Invoices UI (invoices.js)', () => {
    ...
    it('should fetch and render invoices on initialization', async () => {
        ...
        expect(apiFetch).toHaveBeenCalledWith('/api/orders/history?page=1&limit=10');
        expect(apiFetch).toHaveBeenCalledWith('/api/orders/history?page=1&limit=10&search=&sortBy=created_at&sortOrder=desc');
    ...
    it('should redirect on retry payment', async () => {
        ...
        expect(window.location.href).toBe('/checkout?retryOrder=5');
    });

    it('should render pagination controls if there are multiple pages', async () => {
        apiFetch.mockResolvedValueOnce({ data: [{ id: 1 }], pagination: { currentPage: 1, totalPages: 2 } });
        ...
        expect(apiFetch).toHaveBeenCalledWith('/api/orders/history?page=2&limit=10');
        expect(apiFetch).toHaveBeenCalledWith('/api/orders/history?page=2&limit=10&search=&sortBy=created_at&sortOrder=desc');
    });
});