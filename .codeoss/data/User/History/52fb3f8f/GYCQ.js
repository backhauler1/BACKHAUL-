...
// Configure Multer to upload COMPLIANCE documents directly to S3
...
// Configure a SEPARATE Multer instance for company THUMBNAILS
const uploadThumbnail = multer({
    storage: multerS3({
        ...
        key: function (req, file, cb) {
            const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
            cb(null, `companies/thumbnails/${uniqueSuffix}-${file.originalname}`);
        }
    }),
...
const companyRegistrationSchema = z.object({
    ...
    // This validation ensures the checkbox was checked on the frontend
    privacyPolicy: z.enum(['on'], { errorMap: () => ({ message: 'You must agree to the privacy policy.' }) })
});

/**
 * POST /api/companies/register
 * Registers a new company for the authenticated user.
 */
router.post('/register', protect, uploadThumbnail.single('thumbnail'), validate({ body: companyRegistrationSchema }), async (req, res) => {
    ...
    try {
        ...
        const query = `
            INSERT INTO companies (name, description, location, thumbnail_url, owner_id, location_lng, location_lat)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
            RETURNING *;
        `;
        const values = [companyName, description, location, thumbnailUrl, ownerId, longitude, latitude];
        
        const { rows } = await pool.query(query, values);

        res.status(201).json({ message: 'Company registered successfully!', data: rows[0] });
    } catch (error) {
...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        let queryStr = `SELECT c.*, u.name as owner_name FROM companies c LEFT JOIN users u ON c.owner_id = u.id`;
        const queryParams = [];
        const countParams = [];

        if (search) {
            countQuery += ` WHERE c.name ILIKE $1`;
            queryStr += ` WHERE c.name ILIKE $1`;
        ...
        queryStr += ` ORDER BY c.created_at DESC LIMIT $${queryParams.length + 1} OFFSET $${queryParams.length + 2}`;
...
router.put('/:id', protect, uploadThumbnail.single('thumbnail'), async (req, res) => {
    ...
    try {
        const companyQuery = await pool.query('SELECT owner_id, thumbnail_url FROM companies WHERE id = $1', [companyId]);
        const company = companyQuery.rows[0];
        
        if (!company) return res.status(404).json({ message: 'Company not found.' });
        
        if (company.owner_id !== ownerId && !isAdmin) {
            return res.status(403).json({ message: 'Not authorized to update this company.' });
        }

        if (thumbnailUrl && company.thumbnail_url) {
        ...
        const newThumbnail = thumbnailUrl || company.thumbnail_url;
        const { rows } = await pool.query(
            'UPDATE companies SET name = $1, description = $2, location = $3, thumbnail_url = $4 WHERE id = $5 RETURNING *',
            [companyName, description, location, newThumbnail, companyId]
        );

        res.status(200).json({ message: 'Company updated successfully.', data: rows[0] });
    } catch (error) {
        console.error('Update Company Error:', error);
        res.status(500).json({ message: 'Internal server error while updating company.' });
    }
});
 ...
 */
router.post('/:id/documents', protect, validate({ params: numericParamSchema('id') }), uploadComplianceDoc.single('document'), async (req, res) => {
    const companyId = req.params.id;
 ...
 * Suspends or unsuspends a company.
 */
router.patch('/:id/suspend', protect, authorize('admin'), async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ 
            message: `Company successfully ${suspend ? 'suspended' : 'unsuspended'}.`,
            data: rows[0] 
        });
    } catch (error) {
        console.error('Suspend Company Error:', error);
        res.status(500).json({ message: 'Internal server error while updating company status.' });
    }
});

/**
 * POST /api/companies/find
 * Searches for non-suspended companies.
 */
router.post('/find', async (req, res) => {
    const searchTerm = req.body['search-term'] || '';

    try {
        let queryStr = 'SELECT * FROM companies WHERE is_suspended = false';
        const queryParams = [];

        if (searchTerm) {
            queryStr += ' AND name ILIKE $1';
