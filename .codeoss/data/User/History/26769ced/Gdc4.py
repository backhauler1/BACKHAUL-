...
@tracking_bp.route('/shipper-acknowledge-delivery/<load_id>', methods=['POST'])
@login_required
def shipper_acknowledge_delivery(load_id):
    user_id = session['user_id']
    conn = get_db()
    cursor = conn.cursor()

    cursor.execute("SELECT user_id, signature_required, shipper_acknowledged FROM loads WHERE load_id = ?", (load_id,))
    load_info = cursor.fetchone()

    if not load_info:
        return jsonify({"error": "Load not found."}), 404

    if load_info['user_id'] != user_id:
        return jsonify({"error": "Unauthorized: Only the shipper can acknowledge this delivery."}), 403

    if load_info['shipper_acknowledged'] == 1:
        return jsonify({"error": "This delivery has already been acknowledged by the shipper."}), 400

    cursor.execute("UPDATE loads SET shipper_acknowledged = 1, shipper_acknowledgment_timestamp = ? WHERE load_id = ?", (time.time(), load_id))
    conn.commit()
    return jsonify({"message": "Delivery successfully acknowledged by shipper."}), 200
...
def delivery_confirmation():
    ...
    # Fetch load details for geofencing and signature requirement
    cursor.execute("SELECT delivery, signature_required FROM loads WHERE load_id = ?", (load_id,))
    load_row = cursor.fetchone()
    if not load_row:
        return jsonify({"error": "Load not found."}), 404

    # If signature is mandatory for this load, it must be present.
    # This check handles both photo-only submissions and empty submissions for signature-required loads.
    if load_row['signature_required'] == 1 and not signature:
        return jsonify({"error": "This delivery requires a signature. Please provide one."}), 400
...
def submit_pod(match_id):
    ...
    # Verify the match exists, the user is the assigned transporter, and get load details
    cursor.execute("""
        SELECT m.load_id, v.user_id as transporter_id, l.signature_required
        FROM matches m
    ...
    return render_template('submit_pod.html', match_id=match_id, load_id=load_id, signature_required=match_info['signature_required'])