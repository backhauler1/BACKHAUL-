    ...
    try {
        ...
        loads.forEach(load => {
            ...
            card.innerHTML = `
                ...
                ${ (!load.status || load.status === 'available') 
                    ? `<button class="btn cancel-load-btn" data-id="${load.id}" style="width: 100%; padding: 8px; border: 1px solid #dc3545; background: transparent; color: #dc3545; border-radius: 4px; cursor: pointer;">Cancel Load</button>` 
                    : `<a href="/loads/${load.id}/status" style="display: block; text-align: center; width: 100%; padding: 8px; box-sizing: border-box; background: #f1f1f1; color: #333; border-radius: 4px; text-decoration: none;">Track Status</a>`
                }
            `;
