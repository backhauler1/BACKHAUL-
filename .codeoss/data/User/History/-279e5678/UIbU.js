...
export async function initializeMyPostedLoadsPage(containerId = 'my-posted-loads-container') {
    ...
    if (!document.getElementById('post-load-section')) {
        ...
        postForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                loadPostedLoads(container, 1); 
                
                // Contextually ask for push notification permission after their first success
                setTimeout(() => {
    ...
    // Add controls container if it doesn't exist
    if (!document.getElementById('my-posted-loads-controls-container')) {
        const controlsContainer = document.createElement('div');
        controlsContainer.id = 'my-posted-loads-controls-container';
        controlsContainer.style.marginBottom = '20px';
        controlsContainer.style.display = 'flex';
        controlsContainer.style.justifyContent = 'space-between';
        controlsContainer.style.alignItems = 'center';
        controlsContainer.innerHTML = `
            <input type="text" id="my-posted-loads-search" placeholder="Search by title..." style="padding: 8px; border-radius: 4px; border: 1px solid #ccc; font-size: 0.9em; flex: 1; max-width: 300px;">
            <select id="my-posted-loads-sort" style="padding: 8px; border-radius: 4px; border: 1px solid #ccc; font-size: 0.9em; cursor: pointer;">
                <option value="pickup_date-desc">Pickup Date (Newest First)</option>
                <option value="pickup_date-asc">Pickup Date (Oldest First)</option>
        ...
        document.getElementById('my-posted-loads-sort').addEventListener('change', () => {
            loadPostedLoads(container, 1);
        });

        let searchTimeout;
        document.getElementById('my-posted-loads-search').addEventListener('input', () => {
    ...
    await loadPostedLoads(container, 1);
}

/**
 * Fetches and renders the loads posted by the user into the grid.
 */
async function loadPostedLoads(container, page = 1) {
    container.innerHTML = '<p class="text-secondary">Loading your posted loads...</p>';

    const sortSelect = document.getElementById('my-posted-loads-sort');
    let sortBy = 'pickup_date';
    let sortOrder = 'desc';
    ...
    const searchInput = document.getElementById('my-posted-loads-search');
    const searchQuery = searchInput ? searchInput.value.trim() : '';

    try {
        const response = await apiFetch(`/api/loads/posted?page=${page}&limit=12&sortBy=${sortBy}&sortOrder=${sortOrder}&search=${encodeURIComponent(searchQuery)}`);
        const loads = response.data;
        const pagination = response.pagination;

        if (!loads || loads.length === 0) {
            container.innerHTML = page > 1 ? '<p>No more loads found.</p>' : '<p style="color: #666; font-style: italic;">You have not posted any loads yet.</p>';
            return;
        }
        ...
        if (pagination && pagination.totalPages > 1) {
            ...
            prevBtn.addEventListener('click', () => loadPostedLoads(container, pagination.currentPage - 1));
            ...
            nextBtn.addEventListener('click', () => loadPostedLoads(container, pagination.currentPage + 1));
        ...
        container.querySelectorAll('.edit-load-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                renderEditLoadForm(card, load, container, page);
            });
        });
            ...
            btn.addEventListener('click', async (e) => {
                ...
                try {
                    ...
                    showNotification('Load cancelled successfully.', 'success');
                        await loadPostedLoads(container, page); // Refresh the list
                } catch (error) {
 ...
 */
function renderEditLoadForm(card, load, container, page) {
    ...
    card.querySelector('.cancel-edit-btn').addEventListener('click', () => {
        loadPostedLoads(container, page); // Re-renders the list to return to view mode
    });
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            loadPostedLoads(container, page); // Refresh to display the new info
        } catch (error) {
