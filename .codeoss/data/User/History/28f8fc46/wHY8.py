...
def create_load(app):
    ...
    def _create_load(user_id, description="Test Load", delivery="123 Main St, Anytown, USA", load_id=None, signature_required=0):
        load_id = load_id if load_id else str(uuid.uuid4())
        with app.app_context():
            conn = sqlite3.connect(app.config['DATABASE'])
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO loads (load_id, user_id, description, delivery, timestamp, signature_required) VALUES (?, ?, ?, ?, ?, ?)",
                (load_id, user_id, description, delivery, time.time(), signature_required)
            )
            conn.commit()
            conn.close()
        return {'load_id': load_id, 'user_id': user_id, 'description': description, 'delivery': delivery, 'signature_required': signature_required}
    return _create_load
