...
export function renderResultsList(results, options = {}) {
    ...
    results.forEach((item, index) => {
        ...
        // Discreetly insert an ad after every 4th result
        // IMPORTANT: Replace the placeholders below with your actual AdSense IDs
        // TODO: Uncomment the block below once you have a live website and approved AdSense account
        /* if ((index + 1) % 4 === 0) {
            const adItem = _createAdSenseElement();
...
/**
 * Creates a DOM element for a Google AdSense "In-feed" ad.
 * @returns {HTMLElement}
 * @private
 */
function _createAdSenseElement() {
    const item = document.createElement('div');
    // Use your standard result item classes so it blends in physically
    item.className = 'result-item result-item--ad'; 
    
    item.innerHTML = `
        <span style="font-size: 0.7em; color: #999; text-transform: uppercase; display: block; margin-bottom: 5px; padding-left: 10px;">Advertisement</span>
    ...
    // Google AdSense requires this push command to initialize dynamically inserted ads
    setTimeout(() => {
        try { (window.adsbygoogle = window.adsbygoogle || []).push({}); } 
        catch (e) { console.error('AdSense injection error:', e); }
    }, 50);
...
function _createCompanyResultItem(company) {
    ...
    if (company.lng !== undefined && company.lat !== undefined) {
        item.addEventListener('click', () => {
            ...
            const currentActive = container.querySelector('.result-item.active');
            if (currentActive) {
