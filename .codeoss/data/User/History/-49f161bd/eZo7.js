...
describe('Admin Users Dashboard (adminUsers.js)', () => {
    ...
    describe('exportSuspensionHistoryCSV', () => {
        ...
        it('should trigger the export API request and download the CSV', async () => {
            ...
            // Explicitly set the search term to an empty string for this test to ensure isolation.
            await loadSuspensionHistory(1, '');
            apiFetch.mockClear().mockResolvedValueOnce(mockBlob); // Clear mocks and set up for the export call

            await exportSuspensionHistoryCSV();

            expect(apiFetch).toHaveBeenCalledWith('/api/users/suspension-history/export', {
                method: 'POST',
                body: { search: '' }, // Verify it uses the now-cleared search term
                responseType: 'blob'
            });
