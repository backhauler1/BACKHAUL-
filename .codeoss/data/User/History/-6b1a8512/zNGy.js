    ...
    cards.forEach(async (card) => {
        ...
        try {
            ...
            if (!startCoords || !endCoords) {
                ...
                document.getElementById(mapContainerId).innerHTML = '<p style="text-align:center; padding-top: 80px; color: var(--text-secondary);">Could not display map.</p>';
                return;
            }
            ...
            document.getElementById(mapContainerId).innerHTML = '<p style="text-align:center; padding-top: 80px; color: var(--text-secondary);">Error loading map.</p>';
        }
    });
}
...
function displayMatches(trucks, resultContainer) {
    ...
    trucks.forEach(truck => {
        ...
        const verifiedBadge = truck.id_verified ? '<span class="badge" style="background-color: var(--success-color); color: var(--white); font-size: 0.7em; vertical-align: middle; padding: 3px 6px; border-radius: 10px; margin-left: 5px;">✓ ID Verified</span>' : '';
...
// --- Load Form Interactions ---
function setupLoadFormInteractions() {
    ...
    if (isTrailerCheckbox && bolGroup) {
        const toggleBolVisibility = () => {
            if (isTrailerCheckbox.checked) {
                ...
                if (bolCheckbox) bolCheckbox.checked = false;
            }
        };
        toggleBolVisibility();
...
function initializeChat(currentUserId, otherUserId) {
    const chatBox = document.getElementById('chatBox');
    const chatForm = document.getElementById('chatForm');
    const messageInput = document.getElementById('messageInput');
    
    // Ensure elements exist and the Socket.IO library is loaded
    if (!chatBox || !chatForm || !messageInput || typeof io === 'undefined') return;
