...
/**
 * Sets up the "Upload Signed BOL" form with file size and format validation.
 * @param {HTMLFormElement} formElement The form element to attach the listener to.
 */
function setupSignedBolForm(formElement) {
    formElement.addEventListener('submit', async (e) => {
        ...
        const formData = new FormData(formElement);
        const file = formData.get('signedBolDocument');
        ...
        // 2. Validate File Format (PDF or common image formats)
        const validTypes = ['application/pdf', 'image/jpeg', 'image/png', 'image/webp'];
        if (!validTypes.includes(file.type)) {
            return showNotification('Invalid file format. Please upload a PDF or an image (JPEG, PNG, WebP).', 'warning');
        }

        // 3. Validate File Size (10MB limit to match backend Multer configuration)
        const maxSizeInBytes = 10 * 1024 * 1024; // 10MB
        if (file.size > maxSizeInBytes) {
            return showNotification('File is too large. The maximum allowed size is 10MB.', 'warning');
        }

        const loadId = formData.get('loadId');
        if (!loadId) {
            return showNotification('System Error: Load ID is missing.', 'error');
        }
        ...
        try {
            const responseData = await apiFetch(`/api/loads/${loadId}/signed-bol`, {
                method: 'POST',
                body: formData, // Send FormData directly for the multipart upload
            });
