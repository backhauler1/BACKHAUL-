...
describe('Order Routes - GET Endpoints', () => {
    ...
    describe('GET /api/orders/history', () => {
        ...
        it('should return 400 if endDate is before startDate', async () => {
            const res = await request(app).get('/api/orders/history?startDate=2023-10-10&endDate=2023-10-01');

            expect(res.statusCode).toBe(400);
            expect(res.text).toContain('End date cannot be before start date.');
        });

        it('should correctly process valid date ranges where endDate >= startDate', async () => {
            mockClient.query.mockResolvedValueOnce({ rows: [{ count: '1' }] });
            mockClient.query.mockResolvedValueOnce({ rows: [] });

            const res = await request(app).get('/api/orders/history?startDate=2023-10-01&endDate=2023-10-31');

            expect(res.statusCode).toBe(200);
            expect(mockClient.query).toHaveBeenCalledTimes(2);
            
            // Verify that the correct date filters and time extensions were appended to the database query
            expect(mockClient.query.mock.calls[0][0]).toContain('AND created_at >= $2 AND created_at <= $3');
            expect(mockClient.query.mock.calls[0][1]).toEqual([1, '2023-10-01', '2023-10-31 23:59:59']);
        });
    });

    describe('GET /api/orders/history/export', () => {
        it('should return a CSV of the user order history', async () => {
            const mockOrders = [
                { id: 101, amount: 5000, currency: 'usd', status: 'succeeded', created_at: new Date('2023-01-01T12:00:00Z').toISOString() }
            ];
            pool.query.mockResolvedValueOnce({ rows: mockOrders });

            const res = await request(app).get('/api/orders/history/export');

            expect(res.statusCode).toBe(200);
            expect(res.headers['content-type']).toContain('text/csv');
            expect(res.headers['content-disposition']).toBe('attachment; filename="invoices.csv"');
            
            // Check that the headers and a mocked row are in the response
            expect(res.text).toContain('Order ID,Date,Amount,Currency,Status');
            expect(res.text).toContain('"101"');
            expect(res.text).toContain('"50.00"');
            expect(res.text).toContain('"USD"');
            expect(res.text).toContain('"succeeded"');
            
            expect(pool.query).toHaveBeenCalledTimes(1);
            expect(pool.query.mock.calls[0][0]).toContain('SELECT id, amount, currency, status, created_at');
        });

        it('should return 400 if endDate is before startDate', async () => {
            const res = await request(app).get('/api/orders/history/export?startDate=2023-10-10&endDate=2023-10-01');

            expect(res.statusCode).toBe(400);
            expect(res.text).toContain('End date cannot be before start date.');
        });

        it('should correctly process valid date ranges and include them in the CSV header', async () => {
            const mockOrders = [
                { id: 101, amount: 5000, currency: 'usd', status: 'succeeded', created_at: new Date('2023-10-15T12:00:00Z').toISOString() }
            ];
            pool.query.mockResolvedValueOnce({ rows: mockOrders });

            const res = await request(app).get('/api/orders/history/export?startDate=2023-10-01&endDate=2023-10-31');

            expect(res.statusCode).toBe(200);
            expect(res.text).toContain('"Date Range:","2023-10-01 to 2023-10-31"');
            expect(pool.query).toHaveBeenCalledTimes(1);
            expect(pool.query.mock.calls[0][0]).toContain('AND created_at >= $2 AND created_at <= $3');
            expect(pool.query.mock.calls[0][1]).toEqual([1, '2023-10-01', '2023-10-31 23:59:59']);
        });
    });
    ...
    describe('POST /api/orders/:orderId/cancel', () => {
        it('should successfully cancel an order, issue a Stripe refund, and send an email', async () => {
            const mockOrder = { id: 101, stripe_payment_intent_id: 'pi_123', status: 'succeeded', amount: 5000, currency: 'usd' };
        ...
        it('should return 400 if the order status is not succeeded', async () => {
            const mockOrder = { id: 102, stripe_payment_intent_id: 'pi_124', status: 'pending', amount: 2500, currency: 'usd' };
            pool.query.mockResolvedValueOnce({ rows: [mockOrder] }); // Order query
