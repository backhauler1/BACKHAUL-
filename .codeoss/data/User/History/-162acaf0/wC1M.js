...
const listQuerySchema = z.object({
    ...
    sortBy: z.enum(['created_at', 'name', 'is_available']).optional().default('created_at'),
...
router.get('/me', protect, authorize('driver'), validate({ query: listQuerySchema }), async (req, res) => {
    const ownerId = req.user.id;
    const { page, limit, sortBy, sortOrder } = req.query;
    const offset = (page - 1) * limit;

    try {
        const countQuery = 'SELECT COUNT(id) FROM trucks WHERE owner_id = $1';
        ...
        if (sortBy === 'name') {
            orderByClause = `name ${sortOrder === 'asc' ? 'ASC' : 'DESC'}`;
        } else if (sortBy === 'is_available') {
            orderByClause = `is_available ${sortOrder === 'asc' ? 'ASC' : 'DESC'}, created_at DESC`;
        } else {
            orderByClause = `created_at ${sortOrder === 'asc' ? 'ASC' : 'DESC'}`;
        }
        ...
        const queryStr = `
            ...
            LIMIT $2 OFFSET $3
        `;
        const { rows: trucks } = await pool.query(queryStr, [ownerId, limit, offset]);
        res.status(200).json({ 
            data: trucks,
...
router.get('/', protect, authorize('admin'), validate({ query: listQuerySchema }), async (req, res) => {
    ...
    try {
        ...
        if (sortBy === 'name') {
            orderByClause = `t.name ${sortOrder === 'asc' ? 'ASC' : 'DESC'}`;
        } else if (sortBy === 'is_available') {
            orderByClause = `t.is_available ${sortOrder === 'asc' ? 'ASC' : 'DESC'}, t.created_at DESC`;
        } else {
            orderByClause = `t.created_at ${sortOrder === 'asc' ? 'ASC' : 'DESC'}`;
        }
        ...
        const queryStr = `
            ...
            ${whereClause}
            ORDER BY ${orderByClause} LIMIT $${paramIndex++} OFFSET $${paramIndex++}
        `;
