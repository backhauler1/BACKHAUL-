...
const validateCsrf = (req, res, next) => {
    ...
    // Check header OR body for the token to support both AJAX and standard forms
    ...
    // It's important to ensure buffers are of the same length before comparing.
    const cookieBuffer = Buffer.from(cookieToken);
    const clientBuffer = Buffer.from(clientToken);

    if (cookieBuffer.length !== clientBuffer.length || !crypto.timingSafeEqual(cookieBuffer, clientBuffer)) {
        return res.status(403).json({ message: 'CSRF token validation failed. Unauthorized request.' });
    }
