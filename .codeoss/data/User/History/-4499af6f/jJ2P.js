...
jest.mock('./geocodingService', () => ({ geocodeAddress: jest.fn() }));
...
describe('Loads API - POST /post', () => {
    ...
    it('should successfully post a new load and attach geocoded coordinates', async () => {
        ...
        expect(pool.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO loads'), expect.any(Array));
    });
...
describe('Loads API - Rating and Review Endpoints', () => {
    ...
    describe('POST /:id/rate', () => {
        it('should successfully submit a rating and update user average', async () => {
            pool.query.mockResolvedValueOnce({ rows: [{ owner_id: 2, driver_id: 1 }] }); // Load check
        ...
        it('should return 400 if a rating has already been submitted', async () => {
            pool.query.mockResolvedValueOnce({ rows: [{ owner_id: 2, driver_id: 1 }] });
            mockClient.query.mockResolvedValueOnce({}); // BEGIN
    ...
    describe('PUT /:id/rate', () => {
        it('should successfully update an existing rating', async () => {
            ...
            mockClient.query.mockResolvedValueOnce({ rows: [{ rating: '4.0' }] });
            mockClient.query.mockResolvedValueOnce({}); // COMMIT
            ...
            expect(mockClient.query).toHaveBeenCalledWith(
                expect.stringContaining('((rating * rating_count) - $1::numeric + $2::numeric) / rating_count'),
                [3, 4, 2]
            );
        });
    ...
    describe('DELETE /:id/rate', () => {
        it('should successfully delete a rating and update user average', async () => {
            ...
            mockClient.query.mockResolvedValueOnce({ rows: [{ rating: '3.0' }] }); // UPDATE users
            mockClient.query.mockResolvedValueOnce({}); // COMMIT
        ...
        it('should correctly reset rating to 0 when the last rating is deleted', async () => {
            ...
            mockClient.query.mockResolvedValueOnce({ rows: [{ rating: '0' }] }); // UPDATE users
            mockClient.query.mockResolvedValueOnce({}); // COMMIT
            ...
            expect(mockClient.query).toHaveBeenCalledWith(
                expect.stringContaining('CASE WHEN rating_count > 1 THEN'),
                [5, 2]
            );
        });
