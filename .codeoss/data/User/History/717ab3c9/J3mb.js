...
// npm install
    ...
    setupGlobalComponents(socket);
    initializePageComponents(socket);
...
/**
 * Checks if the user is authenticated and toggles the UI accordingly.
 */
async function checkAuthAndToggleUI() {
    const authContainer = document.getElementById('app-authenticated');
    const guestContainer = document.getElementById('app-guest');
    
    if (!authContainer && !guestContainer) return;
    
    try {
        // Using an existing generic user endpoint to check auth status
        const response = await apiFetch('/api/users/me'); 
        if (response && response.id) {
            document.body.dataset.userId = response.id;
 ...
 */
function setupGlobalComponents(socket) {
    initializeTheme();
...
/**
 * Sets up global WebSocket listeners for features like unread message badges.
 * @param {Socket} socket - The global Socket.IO instance.
 */
function setupGlobalSocketListeners(socket) {
    if (!socket) return;
// Use morgan for HTTP request logging. The 'dev' format is great for development.
app.use(morgan('dev'));

    const currentUserId = document.body.dataset.userId;
    if (!currentUserId) return; // Only for logged-in users
app.get('/', (req, res) => {
  res.send('<h1>Hello World!</h1><p>Your request has been logged by morgan.</p>');
});

    socket.on('message', (data) => {
        // Don't show a badge for your own messages
        if (String(data.sender_id) === currentUserId) return;

        // Don't show a badge if we are on the chat page with that specific user
        const chatBox = document.getElementById('chatBox');
        if (chatBox && String(chatBox.dataset.otherUserId) === String(data.sender_id)) {
            return;
        }

        // Show the badge on the navigation link
        // NOTE: The selector 'a[href*="chat"]' assumes your navigation link to the chat page contains "chat" in its href.
        // You may need to adjust this to a more specific ID or class like '#nav-chat-link'.
        const messagesLink = document.querySelector('a[href*="chat"]');
        if (messagesLink && !messagesLink.querySelector('.unread-badge')) {
            const badge = document.createElement('span');
            badge.className = 'unread-badge';
            badge.style.cssText = 'position: absolute; top: 2px; right: 2px; width: 10px; height: 10px; background-color: #dc3545; border-radius: 50%; border: 2px solid white;';
            messagesLink.style.position = 'relative'; // Ensure the parent is a positioning context
 ...
 */
function initializePageComponents(socket) {
    if (document.getElementById('chatBox')) initializeChatPage(socket);
    if (document.forms.length > 0) setupAllForms();
    if (document.getElementsByClassName('map-container').length > 0) initializeAllMaps();
    if (
