...
const { validateCsrf, generateCsrfToken } = require('../middleware/csrf'); // Import CSRF middleware

const router = express.Router();

/**
 * Middleware to intercept express-validator results and format error responses.
 */
const handleValidationErrors = (req, res, next) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        // Combine all error messages into a single, comma-separated string for the frontend
...
/**
 * Reusable validation chain for strong passwords.
 * Defaults to checking 'password' but can check fields like 'newPassword'.
 */
const validateStrongPassword = (field = 'password') => [
    body(field)
        .notEmpty().withMessage(`Please provide a ${field}.`)
        .isLength({ min: 8 }).withMessage(`${field} must be at least 8 characters long`)
 ...
 * Provides a CSRF token for the frontend to include in subsequent state-changing requests.
 */
...
router.post('/register', [
    ...
    try {
        ...
        const newUserQuery = await pool.query(
            'INSERT INTO users (name, email, password_hash, password_changed_at) VALUES ($1, $2, $3, NOW()) RETURNING id, name, email, created_at',
            [name, email, password_hash] // password_changed_at is set to the creation time
        );
...
/**
 * Rate limiter for password reset requests to prevent email spamming and abuse.
 */
...
router.post('/reset-password/:token', [
    ...
    try {
        ...
        await pool.query(
            ...
            [password_hash, user.id] // Also update the password_changed_at timestamp
        );
...
router.patch('/profile', protect, validateCsrf, [
...
router.patch('/change-password', protect, validateCsrf, [
    ...
    try {
        ...
        // (Note: Your `protect` middleware intentionally excludes the password_hash from req.user for security)
        ...
        // 3. Verify that the provided current password matches the one in the database
        const isMatch = await bcrypt.compare(currentPassword, user.password_hash);
        if (!isMatch) {
            return res.status(401).json({ message: 'Incorrect current password.' });
        }

        // 5. Hash the new password and update the user's record in the database
        const salt = await bcrypt.genSalt(10);
        const newPasswordHash = await bcrypt.hash(newPassword, salt);
        await pool.query('UPDATE users SET password_hash = $1, password_changed_at = NOW() WHERE id = $2', [newPasswordHash, userId]);

        res.status(200).json({ message: 'Password changed successfully.' });
    } catch (error) {
        console.error('Change Password Error:', error);
        res.status(500).json({ message: 'An internal server error occurred while changing your password.' });
    }
});
