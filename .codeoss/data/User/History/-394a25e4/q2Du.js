...
router.patch('/change-password', protect, async (req, res) => {
    ...
    if (!currentPassword || !newPassword) {
        return res.status(400).json({ message: 'Please provide both your current and new passwords.' });
    }
    ...
    try {
        ...
        // 4. Validate the strength of the new password
        const passwordErrors = [];
        if (newPassword.length < 8) passwordErrors.push('be at least 8 characters long');
        if (!/[A-Z]/.test(newPassword)) passwordErrors.push('contain at least one uppercase letter');
        if (!/[a-z]/.test(newPassword)) passwordErrors.push('contain at least one lowercase letter');
        if (!/[0-9]/.test(newPassword)) passwordErrors.push('contain at least one number');
        if (!/[!@#$%^&*(),.?":{}|<>]/.test(newPassword)) passwordErrors.push('contain at least one special character');

        if (passwordErrors.length > 0) {
            return res.status(400).json({ message: `New password must ${passwordErrors.join(', ')}.` });
        }
