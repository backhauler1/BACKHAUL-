...
router.post('/create-payment-intent', protect, validate({ body: checkoutSchema }), async (req, res) => {
    // Ensure the user is verified before they can initiate a payment
    if (!req.user.id_verified) {
        return res.status(403).json({ message: 'You must verify your identity securely via Stripe before making a payment.' });
    }
    ...
    try {
        ...
        if (promoCode) {
            ...
            if (promoCodes.data.length > 0) {
                ...
                // Manually enforce minimum amount restrictions
                if (promotionCode.restrictions && promotionCode.restrictions.minimum_amount) {
                    if (amountInCents < promotionCode.restrictions.minimum_amount) {
                        const minDollars = (promotionCode.restrictions.minimum_amount / 100).toFixed(2);
                        return res.status(400).json({ message: `This promo code requires a minimum order amount of $${minDollars}.` });
                    }
                }
        ...
        // Gracefully handle known Stripe API errors
