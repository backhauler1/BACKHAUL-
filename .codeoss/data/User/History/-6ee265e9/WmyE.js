...
router.post('/create-payment-intent', protect, validate({ body: checkoutSchema }), async (req, res) => {
    ...
    try {
        ...
        const { amountInCents, trustedItems } = await calculateOrderAmountAndItems(items, isExpress);
                            ...
                            if (orderCount === 1) { // It's their first successful order!
                                ...
                                if (referrer) {
                                    const emailOptions = {
                                        ...
                                        text: `Hi ${referrer.name},\n\nSomeone you referred just completed their first order! As a thank you, here is a $50 promo code for your next order: ${promoCode.code}\n\nBest,\nYour App Team`,
                                        html: `<div style="font-family: sans-serif; line-height: 1.6;"><h2>You earned a $50 Reward!</h2><p>Hi ${referrer.name},</p><p>Someone you referred just completed their first order!</p><p>As a thank you, here is a $50 promo code for your next order:</p><h3 style="background-color: #f4f4f4; padding: 10px; display: inline-block; border-radius: 5px;">${promoCode.code}</h3><br><p>Best,</p><p><strong>Your App Team</strong></p></div>`
                                    };
                                    await sendEmail(emailOptions);
