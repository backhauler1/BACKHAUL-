...
const calculateOrderAmountAndItems = async (frontendItems) => {
    ...
    for (const item of frontendItems) {
        let trustedUnitPriceInCents = 0;
        let description = '';

        // In a real application, you query your database here using the item.id
        // Example: const { rows } = await pool.query('SELECT price_cents, name FROM products WHERE id = $1', [item.id]);
        
        // For demonstration, we'll use a hardcoded lookup map based on a mock catalog.
        if (item.id === 'standard_load') {
            trustedUnitPriceInCents = 15000; // $150.00
            description = 'Standard Load Transport';
        } else if (item.id === 'priority_fee') {
            trustedUnitPriceInCents = 2500;  // $25.00
...
router.post('/create-payment-intent', protect, async (req, res) => {
    ...
    try {
        ...
        await createPendingOrder(req.user.id, paymentIntent.id, amountInCents, 'usd', items || []);
