...
describe('Users API - Public Endpoints', () => {
    ...
    describe('GET /api/users/me/export', () => {
        it('should successfully export user data as a downloadable JSON file', async () => {
            // Mock the 6 sequential/parallel queries
            pool.query.mockResolvedValueOnce({ rows: [{ id: 1, name: 'Test User' }] }); // User profile
            pool.query.mockResolvedValueOnce({ rows: [{ id: 101, title: 'Posted Load' }] }); // Owned loads
            ...
            expect(res.headers['content-disposition']).toBe('attachment; filename="user_data_export.json"');
            
            const body = JSON.parse(res.text);
            expect(body.profile.name).toBe('Test User');
            expect(body.activity.loadsPosted[0].title).toBe('Posted Load');
            expect(body.activity.financialOrders[0].amount).toBe(5000);
        });
