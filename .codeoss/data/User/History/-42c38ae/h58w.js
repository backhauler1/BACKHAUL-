 ...
 */
router.get('/history', protect, validate({ query: paginationQuerySchema.extend({ 
    ...
    try {
        ...
        try {
            ...
            if (endDate) {
                baseWhere += ` AND created_at <= $${paramIndex}`;
                queryParams.push(endDate.length === 10 ? `${endDate} 23:59:59` : endDate);
                paramIndex++;
            }
 ...
 * Exports the order history to a CSV file.
 */
router.get('/history/export', protect, validate({ query: z.object({
    search: z.string().optional(),
    sortBy: z.enum(['created_at', 'amount', 'status']).optional().default('created_at'),
    sortOrder: z.enum(['asc', 'desc']).optional().default('desc'),
    startDate: z.string().optional(),
    ...
    try {
        ...
        const { rows: orders } = await pool.query(queryStr, queryParams);

        let csvHeader = '';
        if (startDate || endDate) {
            csvHeader += `"Date Range:","${startDate || 'All Time'} to ${endDate || 'Present'}"\n`;
        }
        csvHeader += 'Order ID,Date,Amount,Currency,Status\n';
        const csvRows = orders.map(order => {
            const escapeCSV = (val) => `"${String(val).replace(/"/g, '""')}"`;
            return [escapeCSV(order.id), escapeCSV(new Date(order.created_at).toLocaleString()), escapeCSV((order.amount / 100).toFixed(2)), escapeCSV(order.currency.toUpperCase()), escapeCSV(order.status)].join(',');
        });
        ...
        res.status(500).json({ message: 'Internal server error while exporting invoices.' });
    }
});
