 ...
 */
router.get('/:orderId', protect, validate({ params: orderIdParamSchema }), async (req, res) => {
 ...
 */
router.get('/invoice/:orderId/pdf', protect, validate({ params: orderIdParamSchema }), async (req, res) => {
    const { orderId } = req.params;
    const userId = req.user.id;
 ...
 */
router.post('/:orderId/cancel', protect, validate({ params: orderIdParamSchema }), async (req, res) => {
    const { orderId } = req.params;
    const userId = req.user.id;
    ...
    try {
        ...
        const orderQuery = await pool.query(
            `SELECT id, stripe_payment_intent_id, status, amount FROM orders WHERE id = $1 AND user_id = $2`,
            [orderId, userId]
        );
        const order = orderQuery.rows[0];
