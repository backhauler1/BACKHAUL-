...
jest.mock('multer', () => {
    ...
    const mockMw = jest.fn((req, res, next) => {
    ...
    multerWrapper._mockMiddleware = mockMw;
...
const mockMulterMiddleware = multer._mockMiddleware;
...
app.post('/test-upload', uploadLimiter, (req, res, next) => {
    if (mockMulterMiddleware) return mockMulterMiddleware(req, res, next);
    next();
}, (req, res) => {
...
describe('Upload Rate Limiter', () => {
    ...
    it('should bypass multer and return 429 when the upload limit is exceeded', async () => {
        ...
        for (let i = 0; i < 5; i++) {
            const res = await request(app).post('/test-upload').send({});
        ...
        const blockedRes = await request(app).post('/test-upload').send({});
