...
describe('Upload Rate Limiter', () => {
    ...
    it('should bypass multer and return 429 when the upload limit is exceeded', async () => {
        ...
        for (let i = 0; i < 5; i++) {
            const res = await request(app).post('/api/companies/register').send({ companyName: 'Test', location: 'NY', privacyPolicy: 'on' });
        ...
        const blockedRes = await request(app).post('/api/companies/register').send({ companyName: 'Test', location: 'NY', privacyPolicy: 'on' });
