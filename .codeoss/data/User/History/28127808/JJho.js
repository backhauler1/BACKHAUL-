    ...
    io.on('connection', (socket) => {
        ...
        socket.on('send_message', async (data) => {
            ...
            try {
                ...
                // Add more context for better debugging in Sentry/Logger
                const errorContext = {
                    message: error.message,
                    stack: error.stack,
                    socketId: socket.id,
                    data, // The original data payload from the client
                };
                logger.error('Error handling send_message:', errorContext);
        ...
        socket.on('message_delivered', ({ messageId, room }) => {
            // Broadcast to the room that the message was delivered
            io.to(room).emit('message_was_delivered', { messageId });
        });
    });
}
