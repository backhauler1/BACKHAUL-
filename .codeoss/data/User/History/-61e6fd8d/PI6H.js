...
// Polyfill for DataTransfer, which is not implemented in JSDOM
...
import { showOtpModal } from './otpVerification';
...
jest.mock('./otpVerification.js', () => ({
    showOtpModal: jest.fn()
}));
...
describe('Frontend Form Validation (forms.js)', () => {
    ...
    beforeEach(() => {
        ...
        window.location = { ...originalLocation, href: 'http://localhost/', assign: jest.fn(), reload: jest.fn() };
        const origFormData = window.FormData;
        window.FormData = class MockFormData extends origFormData {
            constructor(form) {
                super(form);
                this.form = form;
            }
            get(key) {
                const input = this.form && this.form.querySelector(`input[name="${key}"]`);
                if (input && input.type === 'file' && input.files && input.files.length > 0) return input.files[0];
    ...
    describe('Account Deletion Button', () => {
        ...
        it('should submit deletion if the user types DELETE and confirms, and redirect after success', async () => {
            ...
            // Flush the microtask queue to ensure the async click handler finishes executing
            await Promise.resolve();
            await Promise.resolve();
            await jest.runAllTimersAsync(); // Let async API call resolve and timers run
        ...
        it('should trigger OTP modal if backend requires verification', async () => {
            const btn = document.getElementById('delete-account-btn');
            apiFetch.mockRejectedValueOnce(new Error('Identity verification required'));
            ...
            expect(apiFetch).toHaveBeenCalledWith('/api/users/me', { method: 'DELETE' });
            expect(showOtpModal).toHaveBeenCalledTimes(1);
        });
    });
    ...
    describe('Export Data Button', () => {
        ...
        it('should fetch the export as a blob and trigger a download', async () => {
            ...
            expect(apiFetch).toHaveBeenCalledWith('/api/users/me/export', { method: 'GET', responseType: 'blob' });
            expect(window.URL.createObjectURL).toHaveBeenCalledWith(mockBlob);
            expect(window.URL.revokeObjectURL).toHaveBeenCalledWith('blob:mock-url');
        });

        it('should show an error notification if the export fails', async () => {
            const btn = document.getElementById('export-data-btn');
            apiFetch.mockRejectedValueOnce(new Error('Export Failed'));

            btn.dispatchEvent(new Event('click', { bubbles: true }));
            await jest.runAllTimersAsync();

            expect(showNotification).toHaveBeenCalledWith('Failed to export data: Export Failed', 'error');
        });
    });
});