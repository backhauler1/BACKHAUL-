...
router.get('/estimate', validate({ 
    ...
    try {
        ...
        // 3. Calculate estimated rate based on base per-mile rates
        ...
        const estimatedRate = (distanceMiles * ratePerMile) + baseFee;

        res.status(200).json({
            distance_miles: Math.round(distanceMiles),
            estimated_rate_min: Math.round(estimatedRate * 0.85), // -15% margin
            estimated_rate_max: Math.round(estimatedRate * 1.15), // +15% margin
            suggested_rate: Math.round(estimatedRate)
        });
    } catch (error) {
