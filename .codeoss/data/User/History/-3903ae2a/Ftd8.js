...
const { protect, requireVerification } = require('./auth');
...
router.post('/forgot-password', authLimiter, async (req, res) => {
    ...
    try {
        ...
        const emailOptions = {
            to: email,
            subject: req.t('auth.email.subject'),
            text: req.t('auth.email.text', { name: user.name, resetUrl }),
            html: req.t('auth.email.html', { name: user.name, resetUrl })
        };

        await sendEmail(emailOptions);

        res.status(200).json({ message: req.t('auth.resetEmailSent') });
...
router.post('/reset-password', authLimiter, async (req, res) => {
    ...
    try {
        ...
        // 4. Invalidate the user's token version cache in Redis
        try {
            await redisClient.del(`user:${user.id}:token_version`);
        } catch (redisErr) {
            console.error('Redis DEL error:', redisErr);
        }

        // 5. Send a security notification email
        try {
            const emailOptions = {
                to: email,
 ...
 */
router.post('/change-password', protect, requireVerification, authLimiter, async (req, res) => {
    ...
    try {
        ...
        // Invalidate the user's token version cache in Redis
        try {
            await redisClient.del(`user:${userId}:token_version`);
        } catch (redisErr) {
            console.error('Redis DEL error:', redisErr);
        }
