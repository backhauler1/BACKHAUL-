...
def magic_login():
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            # Send the email using the existing logic
            sender_email = "gottabackhaul@gmail.com"
            sender_password = os.environ.get("GMAIL_APP_PASSWORD")
            
            if sender_password:
                try:
                    # 1. Define the plain text and HTML content for a multipart email
                    text_content = f"""
            text_content = f"""
Hello,
...
"""
                    # 2. Create the EmailMessage object and set headers
                    msg = EmailMessage()
                    msg['Subject'] = "Your GottaBackhaul Magic Login Link"
                    msg['From'] = sender_email
                    msg['To'] = email

                    # 3. Set the plain text and HTML content
                    msg.set_content(text_content)
                    msg.add_alternative(html_content, subtype='html')
                    ...
                    # Don't reveal to the user that the email failed to send for security.
                    pass
...
"""
                    msg = EmailMessage()
                    msg['Subject'] = "Reset your GottaBackhaul Password"
                    msg['From'] = sender_email
                    msg['To'] = email

                    # 2. Set the plain text and HTML content
                    msg.set_content(text_content)
                    msg.add_alternative(html_content, subtype='html')
...
def verify_identity():
    ...
    sender_email = "gottabackhaul@gmail.com"
    sender_password = "aeguqyaastarlnze"
    sender_password = os.environ.get("GMAIL_APP_PASSWORD")
    
    if sender_password:
        try:
            msg = EmailMessage()
            msg.set_content(f"Your Verification PIN is: {pin}\n\nBest,\nYour App Team")
            msg['Subject'] = "GottaBackhaul Verification PIN"
            msg['From'] = sender_email
            msg['To'] = email
    ...
    if is_flagged == 1:
        ...
        if sender_password:
            try:
                ...
                msg['Subject'] = "Action Required: Suspicious Load Flagged"
                msg['From'] = sender_email
                msg['To'] = admin_email
        text_content = (
            f"Security Alert: A newly posted load has been flagged for suspicious geolocation activity.\n\n"
        ...
        send_email(admin_email, "Action Required: Suspicious Load Flagged", text_content)
                ...
                server.send_message(msg)
                server.quit()
                print(f"Admin notified about flagged load {load_id}")
            except Exception as e:
    ...
    if row and row['email']:
        ...
        if sender_password:
            try:
                ...
                msg['Subject'] = f"Delivery Proof Uploaded: {row['description']}"
                msg['From'] = sender_email
                msg['To'] = row['email']
        ...
        send_email(row['email'], f"Delivery Proof Uploaded: {row['description']}", text_content)
                ...
                server.send_message(msg)
                server.quit()
                print(f"Delivery notification email sent to {row['email']}")
            except Exception as e:
    ...
    if emails_to_notify and shipper_row:
        ...
        if sender_password:
            try:
                ...
                msg['Subject'] = f"Action Required: Dispute Opened for '{shipper_row['description']}'"
                msg['From'] = sender_email
        ...
        send_email(", ".join(emails_to_notify), f"Action Required: Dispute Opened for '{shipper_row['description']}'", text_content)
                ...
                server.send_message(msg)
                server.quit()
                print(f"Dispute notification email sent to {emails_to_notify}")
            except Exception as e:
                print(f"Failed to send dispute notification email: {e}")
        ...
        if sender_password:
            try:
                ...
                if status == 'won':
                    ...
                    msg['Subject'] = f"Good News: Dispute Won for '{row['description']}'"
                else:
                    ...
                    msg['Subject'] = f"Update: Dispute Lost for '{row['description']}'"
                msg['From'] = sender_email
                msg['To'] = row['email']
                server = smtplib.SMTP_SSL('smtp.gmail.com', 465)
                server.login(sender_email, sender_password)
                server.send_message(msg)
                server.quit()
                print(f"Dispute resolution email sent to {row['email']}")
            except Exception as e:
                print(f"Failed to send dispute resolution email: {e}")
        if status == 'won':
            text_content = f"Hello,\n\nWe have successfully won the payment dispute for the load: '{row['description']}'.\n\nThe funds will remain in your account and no further action is required from you.\n\nThank you for using GottaBackhaul."
        ...
        for user in brokers_to_revoke:
            ...
            if email and sender_password:
                try:
                    ...
                    msg['Subject'] = "Action Required: Verified Broker Status Revoked"
                    msg['From'] = sender_email
                    msg['To'] = email
                    server = smtplib.SMTP_SSL('smtp.gmail.com', 465)
                    server.login(sender_email, sender_password)
                    server.send_message(msg)
                    server.quit()
                    print(f"Sent broker revocation email to {email}")
                except Exception as e:
                    print(f"Failed to send broker revocation email to {email}: {e}")
            if email:
        ...
        for user in travelers_to_revoke:
            ...
            if email and sender_password:
                try:
                    ...
                    msg['Subject'] = "Action Required: Verified Traveler Status Revoked"
                    msg['From'] = sender_email
                    msg['To'] = email
                    server = smtplib.SMTP_SSL('smtp.gmail.com', 465)
                    server.login(sender_email, sender_password)
                    server.send_message(msg)
                    server.quit()
                    print(f"Sent traveler revocation email to {email}")
                except Exception as e:
                    print(f"Failed to send traveler revocation email to {email}: {e}")
            if email:
                text_content = f"Hello {username},\n\nYour 'Verified Traveler' status on GottaBackhaul has been revoked because your Driver's License is missing from your profile.\n\nPlease log in and upload a valid Driver's License to regain your verified status.\n\nThank you,\nThe GottaBackhaul Team"
