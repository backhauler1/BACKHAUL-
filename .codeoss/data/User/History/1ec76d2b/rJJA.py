...
def register():
    if request.method == 'POST':
        ...
        cursor.execute(
            "INSERT INTO users (user_id, username, email, password_hash, role, privacy_policy_agreed, agreed_to_policy_version, agreement_timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (user_id, username, email, password_hash, role, 1, current_policy_version, time.time())
        ...
        return "Registration successful! Please check your email to verify your account before logging in.", 200
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user['password_hash'], password):
            if 'is_email_verified' in user.keys() and user['is_email_verified'] == 0:
                return "Your email address is not verified. Please check your inbox for the verification link.", 403
