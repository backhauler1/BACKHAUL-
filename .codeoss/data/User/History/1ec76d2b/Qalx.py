...
job_log_handler = logging.FileHandler(os.path.join(LOG_DIR, 'scheduled_jobs.log'))
...
def send_insurance_expiration_warnings():
    ...
    try:
        ...
        if not users_to_warn:
            print(f"No users with insurance expiring in {days_away} days.")
            scheduler_logger.info(f"No users with insurance expiring in {days_away} days.")
        else:
            print(f"Found {len(users_to_warn)} user(s) with insurance expiring soon.")
...
"""
        ...
        print(f"An error occurred during the insurance warning job: {e}")
...
def revoke_invalid_verifications():
    ...
    if brokers_to_revoke:
        ...
        print(f"Revoked 'Verified Broker' status for {len(brokers_to_revoke)} user(s).")
