...
# Import the messaging blueprint and its global typing status dictionary
from messaging import messaging_bp, _typing_status, TYPING_TIMEOUT_SECONDS, cleanup_typing_status, _contact_info_release_events
...
def init_db():
    ...
    """)
    # Safely attempt to add shipper contact info columns
    try:
        cursor.execute("ALTER TABLE loads ADD COLUMN shipper_phone TEXT")
        cursor.execute("ALTER TABLE loads ADD COLUMN shipper_contact_email TEXT")
        cursor.execute("ALTER TABLE loads ADD COLUMN shipper_other_contact TEXT")
    except sqlite3.OperationalError:
        pass
    ...
    """)
    # Safely attempt to add contact_info_released to matches
    try:
        cursor.execute("ALTER TABLE matches ADD COLUMN contact_info_released INTEGER DEFAULT 0")
    except sqlite3.OperationalError:
        pass
    """)
...
def login():
    if request.method == 'POST':
        ...
        # Log the failed login attempt before returning an error
...
def receive_load_details():
    ...
    shipper_contact_email = request.form.get('shipper_contact_email')
    shipper_other_contact = request.form.get('shipper_other_contact')
    ...
    cursor.execute(
        "INSERT INTO loads (load_id, user_id, description, weight, dimensions, pickup, delivery, offer, shipper_phone, shipper_contact_email, shipper_other_contact, shipping_date, timestamp, ip_address, is_flagged) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (load_id, user_id, description, weight, dimensions, pickup, delivery, offer, shipper_phone, shipper_contact_email, shipper_other_contact, shipping_date, time.time(), ip_address, is_flagged)
    )
    conn.commit()
    conn.close()
...
def payment_success():
    ...
    if load_id:
        ...
        # After payment, release contact info to the driver for the matched load
        conn = sqlite3.connect(DATABASE)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        cursor.execute("""
            UPDATE matches SET contact_info_released = 1 
            WHERE load_id = ? AND contact_info_released = 0
        """, (load_id,))
        conn.commit()

        # Fetch shipper contact info to push to driver via SSE
        cursor.execute("SELECT shipper_phone, shipper_contact_email, shipper_other_contact FROM loads WHERE load_id = ?", (load_id,))
        shipper_contact = cursor.fetchone()
        if shipper_contact:
            # Find the driver_id for this load
            cursor.execute("SELECT v.user_id FROM matches m JOIN vehicles v ON m.vehicle_id = v.vehicle_id WHERE m.load_id = ?", (load_id,))
            driver_id = cursor.fetchone()
            if driver_id:
                _contact_info_release_events[(driver_id[0], load['user_id'])] = dict(shipper_contact) # (driver_id, shipper_id)
...
def dashboard():
    ...
    cursor.execute("""
        SELECT m.*, l.description as load_description, l.user_id as shipper_id, l.shipper_phone, l.shipper_contact_email, l.shipper_other_contact,
    ...
    for rm in raw_matches:
        ...
        match_dict['shipper_contact_info'] = {'phone': match_dict['shipper_phone'], 'email': match_dict['shipper_contact_email'], 'other': match_dict['shipper_other_contact']}
...
def edit_profile():
    ...
    if request.method == 'POST':
        ...
        cursor.execute("""
            UPDATE users
            SET bio = ?, contact_info = ?, dot_number = ?, mc_certificate_path = ?, insurance_path = ?, drivers_license_path = ?, insurance_expiration_date = ?, is_traveler_verified = ?, shipper_phone = ?, shipper_contact_email = ?, shipper_other_contact = ?
            WHERE user_id = ?
...
def chat(other_user_id):
    ...
    # Fetch other user's details for the chat header
...
if __name__ == '__main__':
    # Ensure the messaging blueprint's cleanup is started
    import sys
