...
describe('Auth API Endpoints', () => {
    ...
    describe('POST /api/auth/logout', () => {
        it('should clear cookies and invalidate the refresh token in the database', async () => {
            // Mock the DB update for the refresh token
            pool.query.mockResolvedValueOnce({ rowCount: 1 });

            const res = await request(app)
                .post('/api/auth/logout')
                .set('Cookie', ['refreshToken=some_valid_refresh_token']); // Simulate a client sending the cookie

            expect(res.statusCode).toBe(200);
            expect(res.body.message).toBe('Logged out successfully.');

            // Verify cookies are cleared by checking the 'Set-Cookie' header
            const setCookieHeaders = res.headers['set-cookie'];
            expect(setCookieHeaders).toBeDefined();
            // Check that the 'token' cookie is being cleared (expires in the past)
            expect(setCookieHeaders.some(cookie => cookie.startsWith('token=;'))).toBe(true);
            // Check that the 'refreshToken' cookie is being cleared
            ...
            expect(pool.query).toHaveBeenCalledWith(
                ...
                ['some_valid_refresh_token']
            );
        });

        it('should return 200 and clear cookies even if no refresh token is provided', async () => {
            const res = await request(app)
                .post('/api/auth/logout'); // No cookie sent

            expect(res.statusCode).toBe(200);
            expect(res.body.message).toBe('Logged out successfully.');

            // Verify the DB was NOT called since no token was provided
    ...
    describe('POST /api/auth/refresh', () => {
        ...
        it('should successfully issue a new access token for a valid refresh token', async () => {
            ...
            const mockUser = { id: 1, roles: ['user'], token_version: 0 };
            ...
            jest.spyOn(jwt, 'verify').mockImplementation((token, secret, callback) => {
                // We're mocking the callback-style usage in refresh.js
            ...
            // Verify a new access token cookie was set
            const setCookieHeaders = res.headers['set-cookie'];
            expect(setCookieHeaders.some(cookie => cookie.startsWith('token=new_access_token'))).toBe(true);
        ...
        it('should return 403 if the refresh token is not found in the database', async () => {
        ...
        it('should return 403 if the refresh token is expired or has an invalid signature', async () => {
            ...
            const res = await request(app).post('/api/auth/refresh').set('Cookie', ['refreshToken=expired_token']);
            expect(res.statusCode).toBe(403);
