...
function _initializeMap(container) {
    ...
    try {
        console.log('🗺️ Mapbox: Initializing a brand NEW map instance (1 Map Load counted).');
        ...
        if (container.dataset.marker === 'true') {
            ...
            const initialMarker = new mapboxgl.Marker(markerEl)
                .setLngLat([lng, lat])
                .addTo(map);
            currentSearchMarkers.push(initialMarker);
        }
...
export function updateMapMarkers(locations, options = {}) {
    ...
    console.log(`📍 Mapbox: Updating existing map with ${locations ? locations.length : 0} new markers (No new Map Load counted).`);
        ...
        locations.forEach(loc => {
            if (loc.lng !== undefined && loc.lat !== undefined) {
                ...
                if (loc.type === 'load') {
                    ...
                    `;
                } else if (loc.type === 'company') {
                    // Assumes company object has: id, name, description, services (array)
                    const contactUrl = `/contact-company/${loc.id}`; // Example URL
                    const contactButtonHTML = `<a href="${contactUrl}" class="popup-contact-btn" style="display: inline-block; margin-top: 10px; padding: 5px 10px; background-color: #007bff; color: white; text-decoration: none; border-radius: 4px;">Contact Company</a>`;
                    const servicesHTML = loc.services 
                        ? `<p style="margin: 5px 0; font-size: 0.85em; color: #666;"><strong>Services:</strong> ${loc.services.join(', ')}</p>`
                        : '';
                    
                    const suspendedHTML = loc.is_suspended
                        ? `<p style="margin: 5px 0; font-weight: bold; color: #d9534f;">This company is currently suspended.</p>`
                        : '';

                    popupHTML = `
                        <div class="company-popup" style="color: #333;">
                            <h4 style="margin: 0 0 5px 0;">${loc.name || 'Transport Company'}</h4>
                            ${suspendedHTML}
                            ${servicesHTML}
                            <p style="margin: 5px 0; font-size: 0.9em;">${loc.description || 'Details unavailable.'}</p>
                            ${contactButtonHTML}
...
export function drawRoute(geoJSON) {
    ...
    activeMaps.forEach(map => {
        ...
        if (map.getSource(sourceId)) {
            console.log('🛣️ Mapbox: Updating existing route line with setData (No new Map Load counted).');
            map.getSource(sourceId).setData(geoJSON);
        } else {
            console.log('🛣️ Mapbox: Adding initial route source and layer.');
...
function _createCustomMarkerElement(location = {}) {
    ...
    el.style.backgroundImage = 'url("/images/truck-marker.png")';
    el.style.width = '32px';
    ...
    // If the truck is unavailable or the company is suspended, apply a disabled style.
    if (location.isAvailable === false || location.is_suspended === true) {
